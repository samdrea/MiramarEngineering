/**
 * Purpose: 
 * 1. Increment/decrement all 4 lifting fans by the same amount with the dpad up/down buttons
 * 2. Control left and right thrusting fans with joystick
 * 3. Close the contraption if triangle(top) pressed
 * 4. Open the contraption if x(bottom) pressed
 * 
 * References:
...
#define MOTOR_RF_PIN 11 // right front (blue)
#define MOTOR_LB_PIN 6 // left back (purple)
#define MOTOR_RB_PIN 9 // right back (red)

// Thrusting pins
#define leftFanPin 2 //(orange)
#define rightFanPin 3 //(green)

// Creates an instance of stepper class
// Pins entered in sequence IN1-IN3-IN2-IN4 for proper step sequence
// Defines the number of steps per rotation for stepper motor
const int stepsPerRevolution = 50;
Stepper myStepper = Stepper(stepsPerRevolution, 16, 17, 19, 5);
...
// if up button is pressed, increase lifting fans all by increment
  ...
  // the diagonal will move one way
  motor_speed_LF = motor_stop_speed + total_speed_change;

  // the other diagonal will move the other way
  motor_speed_RF = motor_stop_speed - total_speed_change;
  
  // prevent the motors from spining past the max
  if (motor_speed_LF > motor_max_speed) {
    motor_speed_LF = motor_max_speed;
  }
  if (motor_speed_LF < motor_min_speed) {
    motor_speed_LF = motor_min_speed;
  }
  if (motor_speed_RF > motor_max_speed) {
    motor_speed_RF = motor_max_speed;
  }
  if (motor_speed_RF < motor_min_speed) {
    motor_speed_RF = motor_min_speed;
  }

  // set diagonal motors to the same speed
  motor_speed_RB = motor_speed_LF;
  motor_speed_LB = motor_speed_RF;
